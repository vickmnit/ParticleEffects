...
bool moveSnowLayer(Particle& particle,float deltaTime)
{
	particle.position().x += deltaX;
	if(particle.position().x > obstacleposX-CRATE_DIM || particle.position().x < obstacleposX+CRATE_DIM) return true;
	return false;
}
...
int main(int argc, char *argv[])
{
	...
	Vector3D gravity = Vector3D(0.0f,-0.5f,0.0f);
	...
	GLESQuadRenderer* rainRenderer = GLESQuadRenderer::create();
	rainRenderer->setScale(0.05f,0.05f);
	rainRenderer->setTexturingMode(TEXTURE_2D);
	//rainRenderer->setTexture(textureSplash);
	//rainRenderer->enableBlending(true);
	...
	splashRenderer1->setBlending(BLENDING_ALPHA);
	...
	splashGroup1->setCustomUpdate(&moveSnowLayer);
