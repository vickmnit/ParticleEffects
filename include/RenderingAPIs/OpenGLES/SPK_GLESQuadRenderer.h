...
#ifndef H_SPK_GLESQUADRENDERER
#define H_SPK_GLESQUADRENDERER
...
namespace GLES
{
	...
	* </ul>
	*/
	class SPK_GL_PREFIX GLESQuadRenderer :	public GLESRenderer,
											public QuadRendererInterface,
											public Oriented3DRendererInterface
											//public GLExtHandler
	{
		SPK_IMPLEMENT_REGISTERABLE(GLESQuadRenderer)
	...
	public :
		...
		*/
		GLESQuadRenderer(float scaleX = 1.0f,float scaleY = 1.0f);
		...
		* @since 1.04.00
		*/
		static GLESQuadRenderer* create(float scaleX = 1.0f,float scaleY = 1.0f);
	...
	private :
		...
		static void (GLESQuadRenderer::*renderParticle)(const Particle&, const size_t);	// pointer to the right render method

		void render2D(const Particle& particle, const size_t iter);			// Rendering for particles with texture 2D or no texture
		void render2DRot(const Particle& particle, const size_t iter);		// Rendering for particles with texture 2D or no texture and rotation
		void render3D(const Particle& particle, const size_t iter);			// Rendering for particles with texture 3D
		void render3DRot(const Particle& particle, const size_t iter);		// Rendering for particles with texture 3D and rotation
		void render2DAtlas(const Particle& particle, const size_t iter);		// Rendering for particles with texture 2D atlas
		void render2DAtlasRot(const Particle& particle, const size_t iter);	// Rendering for particles with texture 2D atlas and rotation

		void GLCallColorAndVertex(const Particle& particle, const size_t iter);

	};


	inline GLESQuadRenderer* GLESQuadRenderer::create(float scaleX,float scaleY)
	{
		GLESQuadRenderer* obj = new GLESQuadRenderer(scaleX,scaleY);
		registerObject(obj);
		return obj;
	}
		
	inline void GLESQuadRenderer::setTexture(GLuint textureIndex)
	{
		this->textureIndex = textureIndex;
	}

	inline GLuint GLESQuadRenderer::getTexture() const
	{
		return textureIndex;
	}

	inline void GLESQuadRenderer::GLCallTexture2DAtlas(const Particle& particle) const
	{
	...
	inline void GLESQuadRenderer::GLCallTexture3D(const Particle& particle) const
	{
	...
	inline void GLESQuadRenderer::invertModelView() const
	{
		float tmp[12];
		float src[16];
